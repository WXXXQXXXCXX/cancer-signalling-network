...
const Attributes: FC<{attr: NodeAttributes|null}> = ({attr}) => {
  ...
  const hallmark_list = useMemo(() => {
    const hm:JSX.Element[] = [];
    if(!attr) return hm;
    const hm_mask = graph.getNodeAttributes(attr.name).hallmark
    console.log(attr, hm_mask);
    for(let i=0; i<10; i++){
      if(hm_mask & 2<<i){
        hm.push(<ul>{hallmarks[i]}</ul>)
      }
    }
    return hm;
  }, [attr])
